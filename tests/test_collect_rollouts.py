...
class ActionNormalizationTests(unittest.TestCase):
    def test_quotes_numeric_bid_for_focus(self):
        self.assertEqual(collect_rollouts._normalize_browsergym_action('focus(12)'), "focus('12')")

    def test_quotes_numeric_bid_for_fill(self):
        self.assertEqual(collect_rollouts._normalize_browsergym_action("fill(15, '8')"), "fill('15', '8')")

    def test_quotes_numeric_bid_for_click_with_modifiers(self):
        self.assertEqual(
            collect_rollouts._normalize_browsergym_action("click(18, modifiers=['Control'])"),
            "click('18', modifiers=['Control'])",
        )

    def test_leaves_scroll_unchanged(self):
        self.assertEqual(collect_rollouts._normalize_browsergym_action('scroll(0, 300)'), 'scroll(0, 300)')

    def test_leaves_already_quoted_bid_unchanged(self):
        self.assertEqual(collect_rollouts._normalize_browsergym_action("click('13')"), "click('13')")
